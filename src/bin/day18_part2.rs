        ...
        .partition_point(|&idx| {
            let graph = Grid::four_connected(
                ...
                |_: Position, v: Position| {
                    ...
                    (0..n).contains(&vi)
                        && (0..m).contains(&vj)
                        && (blocked[index(vi, vj)] == -1 || blocked[index(vi, vj)] > idx as isize)
                },
            );
            let start = (0, 0).into();
            let end = (70, 70).into();
            let reachable = bfs(&graph, start).find_path_to(end).is_some();
            reachable
        });
    let (i, j) = bytes[ansidx];
    println!("{},{}", i, j);
}
